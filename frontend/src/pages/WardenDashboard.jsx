  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [statsResponse, staffResponse, activitiesResponse] = await Promise.all([
        ...
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-green-600">{dashboardStats.completedGrievances}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm sm:text-base text-gray-600">Closed</span>
              <span className="font-medium text-gray-600">{dashboardStats.closedGrievances}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
          ...
          <div className="space-y-4">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-green-600">{staffOverview.staffOnDuty}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm sm:text-base text-gray-600">Avg. Resolution Time</span>
              <span className="font-medium">{staffOverview.avgResolutionTime}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm sm:text-base text-gray-600">Urgent Matters</span>
              <span className="font-medium text-red-600">{staffOverview.urgentMatters}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm sm:text-base text-gray-600">Staff on Leave</span>
              <span className="font-medium text-red-600">{staffOverview.staffOnLeave}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm sm:col-span-2 lg:col-span-1">
          ...
          <div className="space-y-4">
            {recentActivities.map((activity, index) => (
              <div className="text-sm text-gray-600" key={activity.timestamp + index}>
  ...
  const StaffManagement = () => {
    ...
    useEffect(() => {
      const fetchStaffMembers = async () => {
        setIsLoading(true);
        try {
          const response = await fetch('http://localhost:3000/grievances/staff');
          if (!response.ok) {
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Contact</th>
                    <th scope="col" className="hidden sm:table-cell px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
